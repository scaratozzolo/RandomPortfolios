...
def portfolioyear(portfolio, years):
    """
...
def portfolio(portfolioticks, years):
    ...
    saved_port = []
    saved_port.append(portfolioticks)
    size = len(portfolioticks)
    ...
    for num, portfolio in enumerate(saved_port):
        ...
        confyears = portfolioyear(portfolio, years)
        if confyears == []:
            continue
...
if __name__ == '__main__':
    ...
    #CHANGE TICKERS AND YEARS TO YOUR NEEDS, MUST BE LIST
    portfolioticks = ['ALCO','BRS','MSFT','NATR','USNA']
    years = [2012]

    portfolio(sorted(portfolioticks), years)
    returnhist(str(len(portfolioticks)))
#
